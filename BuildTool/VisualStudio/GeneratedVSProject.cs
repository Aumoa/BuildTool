...
class GeneratedVSProject : IGeneratedProject
{
    ...
    private string IntermediateModulePCH;
    ...
    public void Generate(VisualStudioGenerator SlnGenerator)
    {
        ...
        if (!bGenerated)
        {
            ...
            IntermediateModulePCH = "Intermediate/ModulePCH";
    ...
    private List<string> AdditionalIncludeFile = new();
    ...
    private void GenerateRuntime(VisualStudioGenerator SlnGenerator)
    {
        foreach (var Module in DependencyModules)
        {
            ...
            AdditionalIncludeFile.Add(Module.ModuleIncludeFile.FullPath);
...

        {
            ...
            AdditionalIncludeFile = AdditionalIncludeFile.Distinct().ToList();
    ...
    private void GenerateModuleInclude()
    {
        StringBuilder ModuleIncludeBuilder = new();
        ModuleIncludeBuilder.AppendLine("// Module include file, Generated by BuildTool.");
        ModuleIncludeBuilder.AppendLine();
        ModuleIncludeBuilder.AppendLine(CompiledProject.GenerateModuleInclude());

        ModuleInclude = ModuleIncludeBuilder.ToString();
        ModuleIncludeFile = new FileReference(Path.Combine(IntermediateModulePCH, CompiledProject.CompiledRule.Name + ".h"));
    }

    public FileReference ModuleIncludeFile;
    public string ModuleInclude;

    private void GenerateModulePCH()
    {
        StringBuilder ModulePCHBuilder = new();
        ModulePCHBuilder.AppendLine("// Module PCH file, Generated by BuildTool.");
        ModulePCHBuilder.AppendLine();

        foreach (var IncludeFile in AdditionalIncludeFile)
        {
            ModulePCHBuilder.AppendLine($"#include \"{IncludeFile}\"");
        }

        ModulePCH = ModulePCHBuilder.ToString();
        ModulePCHFile = new FileReference(Path.Combine(IntermediateModulePCH, CompiledProject.CompiledRule.Name + ".PCH.cpp"));
    }

    public FileReference ModulePCHFile;
    private string ModulePCH;
    ...
    private void GenerateXml()
    {
        ...
        XmlElement Project = Doc.NewElement("Project");
	    {
		    ...
		    foreach (var Config in Configurations)
            {
                ...
                XmlElement ItemDefinitionGroup = Project.NewElementItemDefinitionGroup(Condition);
			    {
				    XmlElement ClCompile = ItemDefinitionGroup.NewElement("ClCompile");
				    {
                        ...
                        ClCompile.NewElement("ForcedIncludeFiles", ModulePCHFile.FullPath);
                        ClCompile.NewElement("PrecompiledHeader", "Use");
                        ClCompile.NewElement("PrecompiledHeaderFile", ModulePCHFile.FullPath);
		    ...
		    ItemGroup = Project.NewElement("ItemGroup");
		    {
                ...
                {
                    var ClCompile = ItemGroup.NewElementItemInclude("ClCompile", ModulePCHFile.FullPath);
    ...
    public virtual void SaveAll()
    {
        ...
        SaveIfChanged(ModuleIncludeFile, ModuleInclude);
        SaveIfChanged(ModulePCHFile, ModulePCH);
    }

    private void SaveIfChanged(FileReference FileRef, string Source)
    {
        FileRef.GetParent().CreateIfNotExists(true);
        if (!FileRef.IsExist || FileRef.ReadAllText() != Source)
        {
            File.WriteAllText(FileRef.FullPath, Source);
        }
    }
}